...
defmodule GameServer do
  def start(initial_stones_number) do
    spawn(fn -> listen({1, initial_stones_number}) end) |>
    Process.register(:game_server) #names process
  end

  defp listen({nil, 0}), do: listen({1, 30}) #or something else
  ...
  defp do_take({sender, player, num_stones_take, current_stones_count}) when not is_integer(num_stones_take)
  or num_stones_take < 1
  or num_stones_take > 3
  or num_stones_take > current_stones_count do
    send sender, {:error, "You can take from 1-3 stones, and this cannot exceed the total stones in the pile!"}
